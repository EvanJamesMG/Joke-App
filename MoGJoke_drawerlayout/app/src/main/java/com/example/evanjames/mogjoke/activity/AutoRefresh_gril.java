 ...
 */
public class AutoRefresh_gril extends SwipeRefreshLayout implements AbsListView.OnScrollListener{
    /**
     * 滑动到最下面时的上拉操作
     */
     ...
     * @param context
     */
    public AutoRefresh_gril(Context context) {
        this(context, null);
    }

    public AutoRefresh_gril(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setLoading(boolean loading) {
        isLoading = loading;
        if (isLoading) {
            mListView.addView(mListView);
        } else {
            mListView.removeView(mListView);
            mYDown = 0;
            mLastY = 0;
        }
    }
